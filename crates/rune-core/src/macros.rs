...
macro_rules! __cons {
    ($car:expr, $cdr:expr; $cx:expr) => {
        $cx.add({
            let car = $cx.add($car);
            let cdr = $cx.add($cdr);
            unsafe { $crate::cons::Cons::new_unchecked(car, cdr) }
        })
    };
    ($car:expr; $cx:expr) => {
        $cx.add({
            let car = $cx.add($car);
            unsafe { $crate::cons::Cons::new_unchecked(car, $crate::object::nil()) }
        })
    };
}
...
#[macro_export]
macro_rules! __error {
        ($msg:literal $(,)?  $($args:expr),* $(,)?) => ($crate::error::EvalError::new_error(anyhow::anyhow!($msg, $($args),*)));
        ($err:expr) => ($crate::error::EvalError::new($err));
    }
...
macro_rules! __root {
    ($ident:ident, $cx:ident) => {
        rune_core::macros::root!(
            $ident,
            unsafe { rune_core::gc::IntoRoot::into_root($ident) },
            $cx
        );
    };
    ...
    ($ident:ident, $value:expr, $cx:ident) => {
        ...
        let mut root = unsafe { $crate::gc::__StackRoot::new(&mut rooted, $cx.get_root_set()) };
...
macro_rules! __rooted_iter {
    ($ident:ident, $value:expr, $cx:ident) => {
        ...
        let list: $crate::object::Gc<$crate::object::List> = match $value {
            // Convert the value into a list
            value => unsafe { $crate::gc::IntoRoot::into_root(value).try_into()? },
        };
        #[allow(unused_qualifications, unused_mut)]
        let mut $ident = if let $crate::object::List::Cons(head) = list.untag() {
            use $crate::{cons, gc, object};
            ...
            $crate::cons::ElemStreamIter::new(None, None)
        };
    };
}
...
macro_rules! __rebind {
    ...
    ($value:expr, $cx:expr) => {{
        ...
        let unbound = match $value {
            v => unsafe { $crate::object::WithLifetime::<'static>::with_lifetime(v) }
        };
        $cx.bind(unbound)
    }};
}
